...
public class Wechatpay {
    ...
    public static IWXAPI getInstrance(Context context) {
        if (wechatapi == null) {
            wechatapi = WXAPIFactory.createWXAPI(context, Config.WX_PAY, false);
            wechatapi.registerApp(Config.WX_PAY);
        }
        return wechatapi;
    }
    ...
    public static IWXAPI getInstrance2(Context context) {
        if (wechatapi == null) {
            wechatapi = WXAPIFactory.createWXAPI(context, Config.WX_PAY);
            wechatapi.registerApp(Config.WX_PAY);
        }
        return wechatapi;
    }
     ...
     * @param aListener
     */
    public static void setOnRespListener(WXPayClickListener aListener) {
        mListener = aListener;
    }

    //INFO 微信/微信支付
    public static void WechatPay(Context context, PayResponseData entity) {
        ...
        req.packageValue = entity.getPackageStr();//"Sign=" + packageValue;
        req.sign = entity.getSign();
//        req.openId = entity.getOpenId();
        // 在支付之前，如果应用没有注册到微信，应该先调用IWXMsg.registerApp将应用注册到微信
        if (!isWXInstalled(context)) {
            Log.i("wechat_pay", "是否进行注册" + isWXInstalled(context));
            getInstrance(context).registerApp(Config.WX_PAY);
        }

        if(!getPaySupported(context)){
            Toast.makeText(context,"当前微信版本不支持支付，请升级",Toast.LENGTH_SHORT).show();
            return;
        }

        boolean a = getInstrance(context).sendReq(req);

        Log.i("wechat_pay", "发起支付操作" + a);
    }
